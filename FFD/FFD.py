...
class FFD(object):
    ...
    def initFFD(self):
        ...
        [self.minX, self.minY, self.minZ, self.maxX, self.maxY, self.maxZ] = [tmpPoints[0][0], tmpPoints[0][1],
                                                                              tmpPoints[0][2], tmpPoints[0][0],
                                                                              tmpPoints[0][1], tmpPoints[0][2]]
        ...
        self.controlPointsOffset = [
            [[np.array([0., 0., 0.]) for z in range(self.controlPointNumZ)] for y in range(self.controlPointNumY)] for x
            in range(self.controlPointNumX)]
        self.controlPointsPosition = [[[np.array(
            [self.minX + x * self.spaceX, self.minY + y * self.spaceY, self.minZ + z * self.spaceZ]) for z in
                                        range(self.controlPointNumZ)] for y in range(self.controlPointNumY)] for x in
                                      range(self.controlPointNumX)]
        try:
            del self.objectPoints
        except:
            pass
        self.objectPoints = {}
    ...
    def T_local(self, objectPoint):
        [x, y, z] = objectPoint
        [posX, posY, posZ] = [(x - self.minX) / self.spaceX, (y - self.minY) / self.spaceY,
                              (z - self.minZ) / self.spaceZ]
        [i, j, k] = [int(posX) - 1, int(posY) - 1, int(posZ) - 1]
        [u, v, w] = [posX - (i + 1), posY - (j + 1), posZ - (k + 1)]
        res = np.array([0., 0., 0.])
        for l in range(4):
            if 0 <= i + l < self.controlPointNumX:
                for m in range(4):
                    if 0 <= j + m < self.controlPointNumY:
                        for n in range(4):
                            if 0 <= k + n < self.controlPointNumZ:
                                res += self.B[l](u) * self.B[m](v) * self.B[n](w) * \
                                       self.controlPointsOffset[i + l][j + m][k + n]
        return res
